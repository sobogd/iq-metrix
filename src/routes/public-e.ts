...
//
// Wire format is landing's existing lib/analytics.ts client, UNCHANGED —
// only the target origin moves. That client already sends `credentials:
// "include"`, so the `iqr_email` cookie (UI-readable, set on the
// .iq-rest.com apex by dashboard-api) rides along automatically; same-site
// subdomain requests are not subject to SameSite restrictions, only
// same-ORIGIN credential defaults, which the client already opts out of.
const SITE_ID = "iq-rest";
const APP = "landing";
const ALLOWED_ORIGIN = "https://iq-rest.com";
...
export async function publicIngestRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.post("/e", async (request, reply) => {
    ...
    // relay in this system already relies on).
    ...
    if (typeof body.tok === "string" && body.tok) {
      ...
      if (visitId) visitRow = await continueVisit(SITE_ID, visitId, email, now);
    }
    if (!visitRow) {
      const network = clientNetwork(facts.ip);
      const entropy = hashEntropy(facts);
      const hash = sessionHash(await getSalt(SITE_ID), network, ua, entropy);
      visitRow = await resolveVisit(SITE_ID, hash, email, visitSeed(facts), now);
    }
    const visit: Visit = visitRow;

    const contributedMeta = mergeVisitMeta({}, {}, events.map((e) => e.meta));
    await applyIngestSnapshot(visit, { app: APP, meta: contributedMeta }, now);
